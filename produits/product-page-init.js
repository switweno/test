...
// Créer une barre d'achat sticky pour mobile
function setupStickyBuyBar() {
  // Appliquer uniquement sur les petits écrans
  if (window.innerWidth > 768) return;
  ...
  stickyBar.innerHTML = `
    ...
    <button class="btn sticky-order-now"><i class="fab fa-whatsapp"></i> Commander</button>
  `;
  ...
  const stickyOrderBtn = stickyBar.querySelector('.sticky-order-now');
  const orderNowBtn = document.querySelector('.order-now-btn');
  
  if (stickyOrderBtn && orderNowBtn) {
    stickyOrderBtn.addEventListener('click', function() {
      orderNowBtn.click();
    });
  }
  
  // Afficher/masquer la barre en fonction de la position de défilement
  ...
  // Ajouter les styles CSS pour la barre sticky
